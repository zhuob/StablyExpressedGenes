...
library(plyr)
ls <- list(#GSE32216, 
           GSE37159, GSE38879, #GSE39214, GSE42968,
           GSE43865, GSE48767,
...
# if all norm.factors change by the same amount, it will only affect the 
# intercept of (1) in manuscript. So I will try in a random way.
set.seed(10)
random.factor <- runif(length(norm.factors), -0.2, 0.2) + 1 # uniform random factors
norm.factors <- norm.factors*random.factor

nb.data <- prepare.nb.data(stable, norm.factors=norm.factors)
off.set <- as.numeric(nb.data$eff.lib.sizes)

trt <- as.factor(c(trt.GSE37159, trt.GSE38879,# trt.GSE42968,
                   trt.GSE43865,
                   trt.GSE48767, trt.GSE51119, trt.GSE51772, trt.GSE53078, trt.GSE58082, 
                   trt.GSE57806))
n.obs <- c(length(trt.GSE37159), length(trt.GSE38879), #length(trt.GSE42968),
...
write.table(var.comp, "/Users/Bin/Dropbox/Zhuo/Research/Project2014/Result/seedling_9lab_random_normalization_Rsubread.txt")
...
Nonorm <- read.table("seedling_9lab_No_normalization_Rsubread.txt")
randomize <- read.table("seedling_9lab_random_normalization_Rsubread.txt")
AH <- read.table("seedling_9lab_Rsubread.txt")


cor(Nonorm[, 7], AH[, 7], method="spearman")
cor(randomize[, 7], AH[, 7], method="spearman")

nobs <- 2000
Nonorm_AH <- intersect(AH$gene[AH$rank.sum <= nobs],
                        Nonorm$gene[Nonorm$rank.sum <= nobs])
length(Nonorm_AH)

random_AH <- intersect(AH$gene[AH$rank.sum <= nobs],
                       randomize$gene[randomize$rank.sum <= nobs])
length(random_AH)
...
Nonorm.100 <- intersect(Nonorm$gene[Nonorm$rank.sum <= nobs],
                        ref.100[, 1])
length(Nonorm.100)

AH.100 <- intersect(AH$gene[AH$rank.sum <= nobs],
                        ref.100[, 1])
length(AH.100)


random.100 <- intersect(randomize$gene[randomize$rank.sum <= nobs],
                        ref.100[, 1])
length(random.100)

plot(x=Nonorm[, 7], y= AH[, 7], pch=20)
head(Nonorm)
head(AH)
