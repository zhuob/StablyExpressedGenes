...
Results
1. Stably expressed genes in three tissue types.
...
The multi-mapping or multi-overlapping reads were not counted.
  ...
  (\textbf{Three groups}) It is shown (\citep{czechowski2005genome}, \citep{hruz2011refgenes}, and
   \citep{dekkers2012identification}) that transcriptomes vary across different tissue types or development stages. For this reason, we designed three types of experiments: the seedling experiment (only arabidopsis seedling are included) , the leaf experiment (only leaves are included)\label{Seedling}, and the multiple-tissue experiment (this include: shoot apical,  root tip, primary root, inflorescences and siliques, hypocotyl, flower, carpels, aerial tissue).
   
   
   (\textbf{Selecting samples}) We pre-screened experiments by data quanlity and treatment structures: 1. The experiment data should have all samples with at least 70\% successfully assigned reads. 2. If there are repeated measurements over time, we chose only one time period (this includes GSE39463 and GSE43865); 3. We chose experiments with DESeq normalization (discussed in Normalization section) factors ranging from 0.70--1.30 after we had all the read counts; that is, if normalization factors fall outside (0.7, 1.3) for at least one sample, the corresponding experiment is discarded.  Eventually, we obtained 23 data sets (with 209 biological samples): 10 experiments (70 samples) for seedling group (aged 2 days to 10 days), 5 experiments (60 samples) for leaf group and 8 experiments (79 samples) for multiple-tissue group. Samples from different experiments were merged by their unique gene IDs, and then stored as read count matrices. A brief description of the data sets can be found in supplementary material. More details for each experiment are also accessible at NCBI via the unique accession number provided.
...
We used the method described in section \ref{OurMethod} to identify stably expressed
genes in three groups of experiments described in Method section (REF) \ref{Seedling}: the
group of seedling experiments, the group of leaf experiments, the group of
experiments on different tissue types.  (group 1 consists of 70???
samples from 10 experiments on Arabidopsis seedlings, group 2 consists 60
samples from 5 experiments on Arabidopsis leaves, group 3 consists ??? samples
from different 8 experiments on different tissues types.  We summarized the
top 1000 most stably expressed genes in each group in Tables 1-3??? in the
online supplementary materials. ((???) 
The normalized count values  (i.e. read counts
multiplied by a factor of  $10^6$/library sizes, \cite{anders2013count}) give
a \textit{Count per Million} (CPM) interpretation. The mean CPM (averaged across all samples for each gene) of top 1000 vary
from 2.0 to 499.0 for seedling, from 0.2 to 693.4 for leaves, and 0.4 to 278.8, constituting a broad range of expression values.)

Comparison of the lists of top 1000 genes in the three groups reveals that
they share 106 genes in common (supplement material for detail). Those genes
are stably expressed under different experimental conditions and tissue types,
and thus may be worth further study. AT1G13320 was identified by
\cite{hong2010identification} as a stably expressed gene under all six
experimental conditions, and it was also found to be stable in 6 of 10 sets
in \cite{czechowski2005genome}.  Here, AT1G13320 is ranked 446, 112, 687 for
the three sets we examined, making it a good candidate for reference gene.
(??? look at the lists identified in \cite{czechowski2005genome} and see whether
there are common ones: NO, AT1G13320 is the only one that shows up in 6 of 10 sets)

[\textbf{How does stably expressed genes compare to each other?}]
Next, we compared our stably expressed genes to reference sets identified and
validated by variants of PCR expression analyses (e.g., qRT-PCR in
\cite{czechowski2005genome} and RT-qPCR in \cite{dekkers2012identification})
under microarray studies. We randomly chose 5 stably 
expressed genes from Seedling data (AT1G30300, AT1G71270, AT3G03940,
AT4G00420, AT5G20920) to compare gene expression level with 5 traditional HKGs
(AT1G13440, AT3G18780, AT4G05320, AT5G12250, AT5G60390) and 5 reference genes
for developmental series (AT1G13320, AT5G59830, AT2G28390, AT4G33380 and
AT4G34270, Figure 1 of \cite{czechowski2005genome}). Figure
...
We used multiple-tissue data to compare stably expressed genes by Dekkers et. al., which were identified specifically for Arabidopsis seed. Figure \ref{expressinlevel2} shows that seed-specific reference genes are not among the most stably expressed genes. 

 \begin{figure}[H]
\begin{center}
	\includegraphics[scale=0.4]{../Figures/A1.png}
	\includegraphics[scale=0.4]{../Figures/A2.png}
	\includegraphics[scale=0.4]{../Figures/A3.png}
	\caption{{\small{\label{expressinlevel1} Expression levels  count-per-million (CPM) of genes across RNA-Seq samples (seedling data):  5 traditional reference genes (top)};  5 stably expressed genes by Czechowski et. al. (middle) ; 5 stably expressed genes identified by our method (bottom).  }}
\end{center}
\end{figure} 
 ...
 \begin{figure}[H]
 \begin{center}
	%\includegraphics[scale=0.5]{../Figures/B1.png}
	\includegraphics[scale=0.4]{../Figures/B2.png}
	\includegraphics[scale=0.4]{../Figures/B3.png}
	\caption{{\small{\label{expressinlevel2} Expression levels  count-per-million (CPM) of genes across RNA-Seq samples (multiple tissue data): 5 stably expressed genes from Dekkers et. al. (top) ; 5 stably expressed genes identified by our method (bottom) }}}
\end{center}
\end{figure} 

\subsection{Difference between stably expressed genes}
 
 In terms of stability measures, there are key differences between our model, geNorm, NormFinder and CV. In our model, genes are considered to be stably expressed when they have low variance in the estimated log relative mean counts; the geNORM algorithm identifies genes with the most typical expression pattern as stably expressed; the NormFinder finds genes that are stably expressed in terms of log-transformed read count; and CV measure tends to identify genes with small ratio of standard error to expression value as stably expressed.
...
% One nice feature about random effects model (\ref{q1}) is that it allows us to
...
total variation attributable to each the three components averaged over all
genes.  Figure \ref{fig:densityplot} shows the histograms of the percentages.
Figure \ref{fig:all} shows the percentages for a few randmomly selected genes.
...
It also shows that expression levels are less variable within seedling (left)
or leaves (middle) than across different tissue types (right panel) since the
overall variances are larger when different tissues are considered.  
...
\begin{center}
\includegraphics[scale=0.3]{../Figures/var_dens1.png}
\includegraphics[scale=0.3]{../Figures/var_dens2.png}
\includegraphics[scale=0.3]{../Figures/var_dens3.png}
...
One goal of identifying stably expressed genes is to use them as reference genes for normalization. As a justification, we used stably expressed genes to see how normalization factors vary by choosing different size of reference sets.   We chose top 10, 100, 1000, 10000 stably expressed genes as reference, and then calculated normalization factors by DESeq method for each sample in Seedling, Leaf and Tissue data, respectively. Figure \ref{fig:normfactor} shows paired scatter plot of normalization factors, in which high consistency is reached when top 100 or 1000 stably expressed gene are selected. We recommend to use as reference top 1000 stably expressed genes to compute normalization factors. %A common reference set of stably expressed genes will help improve comparability of the data sets and interpretability of DE test results.



 \begin{figure}[h!]
\begin{center}
\includegraphics[scale=0.3]{../Figures/norm1.png}
\includegraphics[scale=0.3]{../Figures/norm2.png}
\includegraphics[scale=0.3]{../Figures/norm3.png}
\includegraphics[scale=0.3]{../Figures/norm4.png}
\caption{\label{fig:normfactor} matrix plot of normalization factors by choosing top 10--10000 stably expressed genes for Seedling (top left), Leaf (top right), Tissue (bottom left). The bottom right plot shows the normalization factors of a new seedling experiment GSE66666 (sample size is 6) when using top 10--10000 stably expressed genes from seedling study.}
\end{center}
\end{figure}
...
%\includegraphics[scale=0.4]{../Figures/C2.png}
