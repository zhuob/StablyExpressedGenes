...
since those genes tend to have smaller dispersion \citep{hruz2011refgenes}. Our stability measure is similar to that of NormFinder. However, NormFinder works for normally distributed expression data and requires a minimun of eight samples per treatment \citep{andersen2004normalization}, therefore it will not be discussed in this paper. %since the sample size is small (two or three samples per treatment) in a typical RNA-Seq experiment.
...
\end{enumerate}
 ...
 effect of iterative elimination later in this section.  In Figure \ref{fig:rankVSrank_RNA}, %we use the so-called {\ref recall} curves.
...
% These points should be intuitive and obvious, but it seems that they---especially points 1) and 3)---are not emphasized often enough in practical DE analysis.
...
we randomly choose five genes from top 100 stably expressed genes identified from leaves group, and compared them to five genes randomly chosen from top 100 stably expressed genes of multiple-tissue group. Figure \ref{expressinlevel2} displays the expression profiles in CPM over the five experiments in leaves group. Three genes from multiple-tissue group show large between-experiment variation in leaves group. As we commented earlier, genes that are stable under a wider range of conditions may not be the most stable ones under a narrow set of conditions.
 ...
 \begin{center}
	...
	\includegraphics[scale=0.4]{../Figures/leaf_leaf.png}
