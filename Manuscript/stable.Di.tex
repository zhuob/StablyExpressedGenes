...
We treat the experiment effect $\alpha$ as a random effect because we view the
collected experiments as a random sample from the pool of all Arabidopsis
RNA-Seq experiments. In a DE test, $\beta$ is usually considered as a
fixed-effect term \dots, here \dots
...
The normal assumption for the random effects in equation (\ref{eq:normalassumption}) are the most common in the GLMM settings. The normality is likely a simplification of reality, yet it is a good starting point and should be adequate for quantifying count variation.
