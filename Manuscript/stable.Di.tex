...
\begin{enumerate}
	...
	\citep{ Rpackage}, to the Arabidopsis reference genome \verb|Arabidopsis_thaliana.TAIR10.22.dna.toplevel.fa |
 ...
 \item
     ...
     The annotation file \verb"Arabidopsis_thaliana.TAIR10.22.gtf" is downloaded from Ensembl plat FTP server. The Multi-mapping or multi-overlapping reads were not counted.  
...
(The role of normalization in our method.)

%Count normalization is needed for adjusting differences in sequencing depths or library sizes due to chance variation in sample preparation.
In our model, normalization is used to adjust for the reduction/increase in relative read frequencies due to chance variation in sample preparation. Both TMM \citep{robinson2010scaling} and Anders and Huber's method \citep{anders2010differential} use genes with relatively small observed fold changes as reference genes in normalization. 
%of non-differentially expressing genes simply to accommodate the increased or decreased relative read frequencies of truly differentially expressing genes
We choose Anders and Huber's method to estimate normalization factors.  Briefly, let $y_{ij}$ denote the read count for $i$th gene of $j$th sample ($m$ genes and $n$ samples in total). We first create a pseudo-reference sample, defining gene $i$th expression value by the geometric mean of the same gene over all real samples,
\begin{equation}
 y_{i,0} = (\prod_{j=1}^ny_{i,j})^{1/n},  i=1, \ldots, m. 
\end{equation} 
Next we calculate the median of the fold-changes in relative frequency between sample $j$ and the pseudo-reference sample over all genes,
...
extra-Poisson variation in read counts among samples in the same treatment groups and plays a similar role as the
\textit{over-dispersion} parameter in a negative binomial model
(\citealt{anders2010differential,di2011nbp}). 
The experiment effect, $\alpha_l$, accounts for all sources of variation at
...
experiment-level sources are often difficult to separate statistically.
The experiment effect $\alpha$ is treated as a random term because we view the
collected data sets as a random sample from the pool of all Arabidopsis
RNA-Seq experiments.  
...
The normal models for the random effects are the most commonly in the GLMM
settings. They are likely simplification of reality \ldots; good starting
point; should be adequate for quatifying count variation..


\subsection{Other stability measures}\label{subsection:OtherStabilityMeasure}
For microarray gene expression data, there are many stability measures. These include, among others, the \textit{$M$-value} in geNorm \citep{vandesompele2002accurate}, the \textit{$\rho$ value} in NormFinder \citep{andersen2004normalization}, and the \textit{CV} measure (coefficient of variation, calculated as \textit{standard deviation/mean}) used in \cite{czechowski2005genome} and \cite{dekkers2012identification}. Each of the three measures considers genes associated with smaller values as more stably expressed in its own right. The $M$-value measure for a set of $m_0$ genes works as follows: first, it calculates the \textit{relative variation} of gene $i_1$ to gene $i_2$
by the standard deviation of the ratios of gene $i_1$ to gene $i_2$ (for log-transformed data) across all samples; then the $M$-value for gene $i_1$ is obtained by taking the average of $m_0-1$ relative variations, one for each pair of gene $i_1$ and the remaining $m_0-1$ genes; next, each of the $m_0$ genes is assigned a $M$ value that can be used to rank the genes in terms of stability measure. NormFinder uses a linear mixed model to estimate the between-group and within-group variations from expression values of microarray data, and then combines the two variations by a Bayesian formulation. For a future experiment, the $\rho$-value for gene $i$ is defined as the absolute value of posterior mean plus one standard prediction deviation of that gene. 
